...
class TrackerParams	// parameters for initializing the tracker
{
public:
					TrackerParams();
...
class Tracker	// variables that defines current state of tracker
{
public:
					Tracker(){};
					~Tracker(){ if( _clf!=NULL ) delete _clf; };
	double			update_location(Mat *frame); // track object in a frame;  requires init() to have been called.
	void			update_classifier(Mat *frame); // track object in a frame;  requires init() to have been called.
	bool			init(Mat *frame, TrackerParams *p, ClfParams *clfparams);
