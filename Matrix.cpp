...
template<> void					Matrixu::displayClose(int fignum)
{
	char name[1024]; 
	sprintf(name,"Figure %d",fignum);
...
template<> bool					Matrixu::CaptureImage(CvCapture* capture, Matrixu &res, int color, int flip)
{
	IplImage *img;
...
template<> void					Matrixu::PlayCam(int color, const char* fname)
{
	...
	for( int cnt=0; true; cnt++ )
	{
		...
		frame._keepIpl=true;
		frame.display(1); char q = cvWaitKey(2);
		WriteFrame(w, frame);
...
template<> void					Matrixu::drawPoints(vectori xvect, vectori yvect, int lineWidth, int R, int G, int B)
{
	CvPoint p1, p2;
...
template<> Matrixu				Matrixu::imResize(float r, float c)
{
	...
	Ipp8u* pBuffer = (Ipp8u*)ippiMalloc_8u_C1(_cols,_rows,&(_dataStep));
	IppStatus ippst;
	for( int k=0; k<_depth; k++ )
		ippst = ippiResizeSqrPixel_8u_C1R((Ipp8u*)_data[k],_roi, _dataStep, _roirect, (Ipp8u*)res._data[k], res._dataStep, res._roirect, pc, pr, 0.0, 0.0, IPPI_INTER_LINEAR, pBuffer);
