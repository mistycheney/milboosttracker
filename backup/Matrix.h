...
public:
	...
	void		drawPoints(vectori xvect, vectori yvect, int lineWidth=2, int R=255, int G=0, int B=0);
	...
	static bool	CaptureImage(CvCapture* capture, Matrixu &res, int color=0, int flip=0);
...
template<class T> void				Matrix<T>::PlayVideo( vector< Matrixu > &vid, int wait )
{
...
{
	CvVideoWriter* w = cvCreateVideoWriter( fname, CV_FOURCC('I','Y','U','V'), fps, cvSize(vid[0].cols(),vid[0].rows()), 3 );
